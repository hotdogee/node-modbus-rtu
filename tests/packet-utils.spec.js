...
test('Parse holding registers packet', (t) => {
  const buf = Buffer.from('11 03 06 AE41 5652 4340 49AD'.replace(/\s/g, ''), 'hex')
...
test('Parse holding registers packet with Unsigned int data type', (t) => {
  const buf = Buffer.from('11 03 02 AE41 49AD'.replace(/\s/g, ''), 'hex')
...
test('Parse holding registers packet with ascii data type', (t) => {
  const buf = Buffer.from('11 03 04 5652 5652 49AD'.replace(/\s/g, ''), 'hex')
...
test('Calculate and add CRC to packet', (t) => {
  const buf = Buffer.from('11 03 06 AE41 5652 4340'.replace(/\s/g, ''), 'hex')
